...
for q in qvec:
  datafile = data_dir + ('q%1d_' % q) + datafile_postfix
...
for i, NSL in enumerate(Lambdavec):
    if i==0:
        ...
        xx[NSL] = MO.multiple_observation_results(lambda_posterior_chains,\
                                          ...
                                          RND=xx_prev.RND,\
                                          verbose=True)
    print "Initiating event class with Lambda = %f and RND = %d" % (NSL, xx_prev.RND)
    #
    fout.write('%d\t%d\n' % (NSL, xx[NSL].RND))
fout.close()
...
print plotdir, Lambdavec[1:], xx.keys()
xx2=xx[500]
print xx2.chain_kde_product(np.array([1145]), num=[1])
...
# In[ ]:
figure(int(np.random.random() * 1e5))
...
for n in range(NEVENTS):
    ...
    if n < 5:
        ...
        lc = 'k'
        alpha = 0.2+((n*1.0)/len(range(NEVENTS)))**2
        if n < 10: lab = '$N=%d$' % (n+1)
        else: lab = ''
...
ax.set_yscale('log')
#grid( which='both')
legend(loc='upper right', ncol=3, frameon=False, fontsize=12)
