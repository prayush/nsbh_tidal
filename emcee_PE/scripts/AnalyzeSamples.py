...
def calculate_bias(\
  ...
  confidence_levels=[90.0, 68.26895, 95.44997, 99.73002]):
    ...
    # We plan to store 5 quantities for each parameter:
    # 0: Median recovered parameter value from posterior samples
    # 1: Bias = (X(median) - X(inj))/X(inj)
    # 2: Confidence level lower bound = X(confidence_level_low)
    # 3: Confidence level upper bound = X(confidence_level_up)
    # 4: Confidence interval = (X(confidence_level_up) - X(confidence_level_low))/X(inj)
    # PLUS
