...
if not post_process:
  ...
  if not resume and not auto_resume:
    ...
    while len(p0) < nwalkers:
        p = emcee.utils.sample_ball(np.array([eta_true, Mc_true, chi2_true, Lambda_true]), np.array([eta_stdev_init, Mc_stdev_init, chi2_stdev_init, Lambda_stdev_init]), 1)[0]
    ...
    axs[1,0].axvline(x=options.chi2_min, c='r', ls='--')
    axs[1,0].axvline(x=options.chi2_max, c='r', ls='--')
    axs[1,1].hist(p0.T[3], 20)
    axs[1,1].set_xlabel(r'$\Lambda$')
    axs[1,1].axvline(x=0, c='r', ls='--')
    axs[1,1].axvline(x=Lambda_max, c='r', ls='--') 
  ...
  if pt:
    print 'Using parallel tempering sampler'
    ntemps = 5
