...
% 
In each panel: the dashed red line marks the median or {\it measured} value
...
from $\chibh\in[0, 1]$, source location uniformly in spatial volume,
and its inclination from $\iota\in[0,\pi]$. Note that we only
...
Fig.~\ref{fig:TT_LambdaCIWidths90_0_Lambda_SNR}. This further restricts the
sub-space of all NSBH observations that our populations span, and will need to
be take into account when the number of observations required for tidal
measurements is related to detector observation time.
% 
...
% 
We use the LEA+ model~\cite{Lackey:2013axa} to produce tidal waveforms that
...
about the NS equation of state, we move on to investigate how well do we do with
...
approximation. For every population generated, we replace each event's
parameters by their nearest neighbor on the uniform grid G, which has vertices
...
% 
There are two sources of error that will enter the deductions we will make from
a a single population generated in the manner described above. First, since the
injection parameters are pushed to their nearest neighbor on a grid, we would 
...
B).
% 
{\it For paradigm A}, we find the following: (i) for the softer equations of state
that result in less deformable neutron stars, $15-20$ detections are sufficient
for our median measured value of $\lambdans$ to lie within $10\%$ of the true
value. (ii) For NSBH populations with more deformable NSs
($\lambdans\geq 1000$), the same is achievable within as few as $10$ realistic
...
of state. All of the above is possible within a few years of design
aLIGO operation~\cite{Abadie:2010cfa}, if astrophysical BHs do indeed have
masses $\leq 5M_\odot$ (i.e. in the mass-gap). However, further
restricting $\lambdans$ will require $50+$ NSBH observations.
% 
{\it For paradigm B}, we find the information accumulation to be somewhat slower.
While the quantitative inferences for populations with $\lambdans\geq1000$ are
not affected significantly, we find $\mathcal{O}(10-20)\%$ more events are
required to attain the same measurement accuracy as under paradigm A.
% 
We conclude that within as few as $20-30$ observations of disruptive NSBH
...
Finally, we note that a more recent work that improves upon LEA+ has been
published~\cite{Pannarale:2015jka}, but it
provides only an amplitude model which needs to be (in future) augmented with a
compatible phase model. In addition, accuracy of the underlying numerical
simulations used to calibrate the waveform model used here have not been
verified against independent codes so far. It is therefore difficult to assess
the combined modeling error and its effect on our results. Our results here
are, therefore, limited by the limitations of our waveform model, and presented
with this caveat. However, we expect the combined effect of modeling errors to
{\it not} affect our qualitative conclusions.
