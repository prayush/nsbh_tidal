...
% 
% 
Past (and future) efforts with Advanced LIGO have used (or plan to use) BHBH
...
We fix $\mns=1.35M_\odot$ and $\chins=0$, and explore a range of nuclear
...
hypercubic regular grid in the BH parameter space, with vertices at
$q\in\{2,3,4,5\}$, i.e.
...
}, by sampling the SNR $\rho\in\{20,30,50,70\}$. Our choice of parameters here
...
disruptive NSBH mergers a year with $\rho\geq 20$, $0.07-2$ a year with
...
probability distribution for binary mass ratio $\eta$ recovered for an NSBH
injection with $\mns=1.35M_\odot$, $\chins=0$, $\mbh=5.4M_\odot$, $\chibh=+0.5$
...
for the measured source parameter $X$.}.
% 
...
parameters using BBH templates, we show $90\%$ credible intervals associated
with NSBH mass and spin measurements in Fig.~\ref{fig:CIWidths90_Lambda_SNR}.
...
meaningful so long as the measurement is {\it accurate} to begin with. Due to
...
In order to quantify the relevance of tidal signatures, we define $\arr_X$ as
the ratio between systematic biases and statistical uncertainties associated
with the measurement of parameter $X$. If $\arr_X\gtrsim 1$, incorporating
tidal effects in inspiral and merger modeling would non-trivially improve our
estimation of parameter $X$ from NSBH signals.
% 
...
\textcolor{blue}{%
Credible intervals $(\Delta X)^{90\%}$ measure the precision with which a 
quantity $X$ can be measured. We show credible intervals for
$X=\{\mchirp,\eta,\chibh\}$ in Appendix~\ref{as1:illustrations}. This {\it 
precision}, however, is only meaningful so long as the measurement is {\it
accurate} to begin with. Due to unmodeled tidal effects, parameter measurements
performed on NSBH signals using BBH templates will have systematic biases. If
these biases are too large, using non-tidal BBH templates should severely impair
our
inferencing ability. In order to quantify the relevance of tidal signatures,
we define $\arr_X$ as the ratio between systematic biases and statistical
uncertainties associated with the measurement of parameter $X$. If
$\arr_X\gtrsim 1$, incorporating tidal effects in inspiral and merger modeling
would non-trivially improve our estimation of parameter $X$ from NSBH
signals.
}

We first calculate the ratio of errors $\arr_{\mchirp}$ , and show it in
