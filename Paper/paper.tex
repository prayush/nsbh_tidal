...
However as the signal's $\lambdans$ is increased from $500\rightarrow 2000$, the 
low-mass BH-NS binaries with large spins on the BHs show very large systematic biases
in spin values, that becomes larger than the statistical uncertinty for $\rho\geq30$
and reaches up to $4\times$ the same quantity as $\rho\rightarrow 70$.
...
instrument noise, and not depend on whether we include tidal effects in template
