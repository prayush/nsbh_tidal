...
holes (BH) and neutron stars (NS) are the primary target sources for the second
...
been seen in binaries with stellar companion (inferred from the observed spectrum
of electromagnetic waves emitted by the companion)~\cite{Lewin2010,
Remillard:2006fc,Fragos:2010tm}.
Neutron stars, on the other hand, have had numerous sightings. Thousands have
electromagnetically emitting neutron stars, or pulsars, have been documented,
...
Mixed binaries of black holes and neutron stars, is a yet undetected but 
astrophysically interesting class of systems~\cite{Wex:1998wt,
1991ApJ...379L..17N,Janka1999,Fryer:2015jpa},
which advanced GW detectors are likely to observe~\cite{Abadie:2010cfa}.


\prayush{
Coalescing binaries of compact objects, i.e. black holes (BH) and/or
neutron stars (NS) are the primary target sources for 
aLIGO~\cite{Abbott:2016nhf,LIGOVirgo2016a}.
% 
Compact binary merger rate estimates suggest
...
An important sub-class of these
are NSBH binaries, of which we expect $\mathcal{O}(10)$ GW observations
every year~\cite{Abadie:2010cf}.
...
Gamma-ray Bursts (GRBs)~\cite{eichler:89,1992ApJ...395L..83N,moch:93,Barthelmy:2005bx,
2005Natur.437..845F,2005Natur.437..851G,Shibata:2005mz,Paschalidis2014,
Tanvir:2013}. Under favorable conditions, the NS can get disrupted by
the tidal field of its companion BH. Once disrupted, most of the NS
material falls into the BH over an $\mathcal{O}(1$ms$)$ time-scale,
with the rest forming an accretion disk around the BH. This disk-BH system,
which survives for $0.1-1$s, is hypothesized to drive SGRBs through the
production of relativistic jets~\cite{Foucart:2015a,
Lovelace:2013vma,Deaton2013,Foucart2012,Shibata:2005mz,Paschalidis2014}.
A {\it coincident} detection of both gravitational and electromagnetic
waves (gamma rays) from an NSBH system is therefore a {\it unique}
possibility to confirm this hypothesis~\cite{Abbott:2016wya}.
% 
...
(EoS) of neutron star material. This is what we will focus on in this paper.
% 
GWs emitted by NSBH coalescences carry subtle hints of the material structure
of NSs. During early inspiral, the (weak) tidal field of the companion BH
deforms the star, thereby exciting its resonant oscillation modes. The induced
quadrupolar moment $Q_{ij}$ of the star depends on the
tidal field $\mathcal{E}_{ij}$ of its companion through the relation
$Q_{ij} = -\lambda\mathcal{E}_{ij}$, where $\lambda$ is an EoS-dependent
tidal deformability parameter that is related to NS's dimensionless Love number
$k_2$ and radius $R_\mathrm{NS}$ as $\lambda:=\frac{2}{3G}k_2 R_\mathrm{NS}^5$.
The coherence of GW emission between stellar quadrupolar waves and orbital
waves drains energy from the orbit and increases the inspiral rate~\cite{Flanagan2008}.
We know the leading and next-to
-leading order terms in post-Newtonian (PN) theory that capture this 
effect~\cite{Vines2011}, entering binary phasing at $5$PN order. As the NS gets
disrupted close to binary's merger, the quadrupole moment of the system falls
monotonically. As a result, the emitted GWs are subdued significantly
post-disruption (as compared to what they would be for a 
BBH)~\cite{Kyutoku:2010zd,Lackey:2013axa,Lovelace:2013vma,Foucart:2015a,
Pannarale:2015jia}.
...
orbits there are over which its effect is visible.
% 
\red{
Text about disruption of NSBHs, MOVE EARLIER.
}
\prayush{
...
Studies~\cite{DelPozzo:13,Chatziioannou:2015uea,Agathos:2015a} have shown that
with $20-30$ BNS observations, aLIGO would begin to distinguish between hard,
moderate and soft candidate equations of state. These studies rely on the PN 
...
waveforms could easily dominate over the tidal terms themselves, since there 
are unknown point-particle terms at orders {\it lower} than that at which tidal
effects enter~\cite{Yagi:2014}.
\red{
...
the waveform model of Ref.~\cite{Lackey:2013axa} (henceforth ``LEA+'').
\red{
(describe meaning of the acronym LEA+)
}
Second, we include the effect
...
mass-ratios $q:=\mbh/\mns\in[2,5]$, black hole spin (aligned with orbit)
$\chibh\in[-0.5, +0.75]$, and dimensionless neutron star tidal deformability 
...
For individual events, ignoring tidal effects will affect mass-estimation
only marginally; only for very loud signals (SNRs $\gtrsim 30$) the systematic
biases exceed the statistical uncertainties. Furthermore, detection searches
can ignore tidal effects without loss of sensitivity.
}
\prayush{
...
LIGO instruments, that are expected to come online in $2018-19$.
\red{
