...
faily deformable neutron stars, with actual $\lambdans = 1500$ (left panel) and
$2000$ (right panel). In each case, we calculate the lowest SNR value $\rho_c$ 
in order
for the signal to have enough information that we can constrain $\dlambda$ below
$100\%$. Note that this is a modest requirement on constrainability of $\lambdans$.
In each panel of the figure, we show $\rho_c$ as a function of BH mass and spin.
% 
We immediately note that to fulfil this modest aim with a single observation, 
we need an SNR of $30$ or higher in each detector. Given we have that, we can 
bring $\dlambda$ below $100\%$ for the lowest BH masses and higher spins. To
attain the same for other values of BH parameters, we will need a very loud
signal with $\rho\gtrsim 50$.
