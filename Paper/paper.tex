...
was announced earlier in 2016~\cite{LIGOVirgo2016a}. Emitted more than a billion
year ago, these waves herald the era of observational gravitational wave (GW)
astronomy as they pass through Earth.
% 
Over a $3$-month O1, aLIGO had access to more binary mergers than initial-LIGO
over its entire coincident operation period. Both of aLIGO instruments will be
further upgraded in phases to reach their design sensitivity by $2018-19$, at 
which point we may expect to observe $\sim 70$ binary mergers a 
...
aLIGO. NSBH binaries form an important class of compact binaries, and we expect
to see $\mathcal{O}(10)$ of these every year with design-sensitivity 
...
Tanvir:2013}. Plausible mechanisms have been proposed for such a 
scenario~\cite{Paschalidis2014,Ciolfi:2014yla}. A joint detection of GWs
from an NSBH system with a GRB would validate this hypothesis.
% 
Another question that NSBH mergers can be used to probe is `what is the 
...
oscillation modes. This drains energy from the orbit resulting in an
increase of the inspiral rate. We know the leading and next-to-leading
order terms in post-Newtonian (PN) theory that capture this 
effect~\cite{Vines2011}, entering binary phasing at $5$PN. Closer to merger, 
the neutron star could get disrupted by the tidal field of its companion.
Once disrupted, the neutron star material may form an accretion disk around 
the black hole. Most of this matter disk spirals in within a few 
milliseconds, with a small fraction becoming unbounded and gets 
ejected~\cite{Foucart2015,Lovelace:2013vma,Deaton2013,Foucart2012,
Paschalidis2014} (under favorable conditions, and possible accompanied by 
gamma-ray emission).
The emitted GW signal is, as a result, heavily damped post-disruption, 
and the quasi-normal modes of the resulting black hole are not excited. Such a
GW signal will be qualitatively very different from that from an equivalent
binary black hole (BBH) merger. The primary goal of this work is to understand
what it requires to optimally extract information about neutron star structure
from such NSBH signals.
...
% 
It is worth noting
that not all point-particle terms in PN theory are known up to such high order,
and therefore we need to be careful when using PN waveforms to measure tidal
effects.
%
...
study has been published~\cite{Pannarale:2015jka}, but it gives only an 
amplitude model and needs to be (in future) augmented with a compatible phase
model. In addition, accuracy of the underlying numerical simulations used to
calibrate the waveform model used here has not been verified against 
independent codes so far.
It is therefore difficult to assess the combined modeling error and the 
results presented in our study are limited by the limitations of the model.
...
% 
\begin{equation}
 \tilde{h}_\mathrm{NSBH}(f, \vec{\theta}) = \tilde{h}_\mathrm{BBH}(f, \vec{\theta} - \{\lambdans\})\,A(f, \vec{\theta})\,e^{\ii \Delta\Phi(f, \vec{\theta})},
\end{equation}
where $\vec{\theta}:=\{\mbh, \mns, \chibh,\chins=0, \lambdans\}$ are the intrinsic binary
...
$\Delta\Phi(f, \vec{\theta})$ is set to the PN tidal phasing corrections,
