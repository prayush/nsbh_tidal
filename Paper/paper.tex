...
\begin{figure*}
\centering
\includegraphics[width=1.8\columnwidth]{plots/TNChiBHBiasesOverCIWidths_CI90_0_Lambda_SNR_linear}
...
% We do so by comparing the systematic biases introduced by this 
...
% \red{Reorder:
% We construct NSBH waveforms with LEA+. We inject them, and run MCMC on the injections
% using BBH templates. We perform this analysis for [... parameters ...]. We always
% use $\mns=1.35M_\odot$, but explore $\lambdans=\{500,1000,1500,2000\}$.
% }
...
become observable only at relatively large SNRs~\cite{Lackey:2013axa}.

% We simulate NSBH signal waveforms using the LEA+ model, inject them into zero
% noise, and perform fully Bayesian parameter estimation analyses on them to
% estimate their source masses and spins. We set tidal terms in search
% templates to zero, ensuring that they mimic BBH mergers. We perform this
% analysis for a range of systems: with mass ratios $q\in\{2,3,4,5\}$, BH spins
% $\chibh\in\{-0.5, 0, +0.5, +0.75\}$, and NS tidal deformability
% $\lambdans\in\{500, 800, 1000, 1500, 2000\}$. We fix NS mass to $1.35M_\odot$
% and its spin to zero (for signals). In addition, signals are simulated at
% different strengths, i.e. $\rho=\{20, 30, 50, 70\}$, in order to estimate the
% dependence of parameter measurement errors on the SNR.
...
We show this ratio of errors $\arr$ associated with chirp mass measurements in
Fig.~\ref{fig:TN_chirpMassBias_vs_Lambda_SNR}. $\mchirp$ is the mass combination
that occurs at leading order in the perturbative expansion of GW strain emitted
...
if additionally: (i) the BH has prograde spin $\chibh\gtrsim 0.4$, {\it and} (ii)
the NS's true deformability is large enough, s.t. $\lambdans \gtrsim 1000$.
% 
At even higher SNRs, say $\rho=70$, the ratio of errors $\arr$ is large
($\simeq 0.5-1.5$) across most of the binary parameter space; and $\mchirp$ measurement
is affected at a $50\%$ or higher level by the use of point-particle waveforms as
templates.
% 
We conclude that only for $\rho\gtrsim 50$ do we need to include in our templates
...
% 
As a last note, we find that $\arr$ is always positive for $\mchirp$, suggesting that
...
In the next figure, Fig.~\ref{fig:TN_EtaBias_vs_Lambda_SNR}, we show the ratio of
measurement errors $\arr$ for the symmetric mass-ratio $\eta$. Note that $\eta$
...
lack of $\lambdans$-dependent terms in the filter template model. This bias,
similar to the $\mchirp$ bias, decreases the merger frequency for BBH templates,
in order to better match the relatively quiet merger and ringdown of NSBHs.
At SNR $\rho=30$
