...
Eq.~\ref{eq:marginalize} to obtain the marginalized probability distribution
for the tidal deformability of the NS $p(\lambdans|d_n,K)$.
The most likely value of $\lambdans$ for this distribution is our 
{\it measured} value for the parameter, and the $90\%$ confidence intervals
associated with the distribution serve as statistical error-bars.
% 
% We use this
% distribution to put confidence bounds on our measurement of $\lambdans$.
% 
% of this distribution to estimate our statistical and 
...
\begin{figure*}
\centering 
\includegraphics[width=1.8\columnwidth]{plots/SingleSystemEta_q3_0_mc1_98_chi0_50}
\caption{$\eta$
}
...
\begin{figure*}
\centering 
\includegraphics[width=1.8\columnwidth]{plots/TNEtaCIWidths90_0_Lambda_SNR}
\caption{$\eta$
}
\label{fig:SingleSystemEtaPDFvsSNR}
\end{figure*}
...
tidal effects.
For all of the resulting signals, we perform a fully Bayesian parameter estimation
aimed at measuring binary masses and spins from the signal. Our analysis
templates differ from the signal in only one sense: all tidal terms are set to zero
in the former, i.e. they correspond to BHBH mergers. Binary mass-ratio is sampled
...
parameters, and rescaled to different strengths with $\rho=\{20, 30, 50, 70\}$.
}
