...
\begin{figure*}
\centering 
\includegraphics[width=0.62\columnwidth]{plots/McEta2D_q4_mc2_25_chi0_50_snr30}
\includegraphics[width=0.62\columnwidth]{plots/EtaChiBH2D_q4_mc2_25_chi0_50_snr30}
\includegraphics[width=0.62\columnwidth]{plots/McChiBH2D_q4_mc2_25_chi0_50_snr30}\\
\includegraphics[width=0.62\columnwidth]{plots/LambdaMc2D_q4_mc2_25_chi0_50_snr30}
\includegraphics[width=0.62\columnwidth]{plots/LambdaEta2D_q4_mc2_25_chi0_50_snr30}
\includegraphics[width=0.62\columnwidth]{plots/LambdaChiBH2D_q4_mc2_25_chi0_50_snr30}\\
\includegraphics[width=0.6\columnwidth]{plots/LambdaMc2D_q4_mc2_25_chi0_50_snr50}
\includegraphics[width=0.6\columnwidth]{plots/LambdaEta2D_q4_mc2_25_chi0_50_snr50}
\includegraphics[width=0.6\columnwidth]{plots/LambdaChiBH2D_q4_mc2_25_chi0_50_snr50}
\caption{\prayush{INCOMPLETE..}We illustrate here two-dimensional probability distributions, showing
the correlation between the NS tidal deformability parameter $\lambdans$ 
and binary mass and spin parameters.
}
...
strength (SNR) required to constrain $\lambdans$ of the companion NS within a factor
of $100\%$ of its true value, i.e. with $\pm 50\%$ error-bars. The NS mass is fixed
at $1.35M_\odot$, spin at zero, and $\lambdans=2000$.
% 
We can see that, irrespective of BH properties, we cannot constrain $\lambdans$
to better than $\pm 50\%$ if the SNR is below $\approx 29$.
% 
...
its companion hole's tidal field becomes discernible in GW spectrum only when
...
differences between NSBH and BHBH signals is maximal.
% 
The third simply makes the NS more easily deformable. All of them also reduce
the onset frequency of NS's disruption, allowing for it to happen earlier in
the orbit. 
% 
...
with LIGO. If, instead, it subtends a non-zero angle with LIGO's line of sight
(say $45^o$), the binary would need to be at $\approx XXX$~Mpc to achieve the
same signal strength.



Ultimately, the extent of tidal modulations of emitted GWs depends on the
equation of state of neutron star interior, which becomes an indirect
...
% 
We calculate here the accuracy with which we measure $\lambdans$ from a 
{\it single} GW observation. We will focus on combining multiple observations
...
% \prayush{%
We sample the same set of systems as in the previous section, i.e. those with
$q=\{2,3,4,5\}$, $\chibh=\{-0.5,0,+0.5,+0.75\}$, and
$\lambdans=\{500, 800, 1000, 1500, 2000\}$. For each unique combination of
these parameters, we generate separate injections at SNRs
$\rho=\{20,30,50,70\}$, using the LEA+ model~\cite{Lackey:2013axa}. We perform a
full Bayesian analyses on each of these injections using LEA+ templates, and obtain
probabilistic estimates for source binary parameters from the GW information
alone, including for the NS tidal deformability parameter $\lambdans$.
...
\red{%
(i) Rework text, so that you slowly build up the information needed. First, explain
the general idea pursued, then discuss one panel first, then introduce second
panel, finally, contrast both panels.
(ii) High information density. Distinction between left and right panels notation
not needed in the first sentence. deferring this info by a few sentences will make
it easier for the reader to follow the discussion.
}
\prayush{%
...
NS with $\lambdans=2000$ one could even put a $\pm 50\%$ error-bar on 
