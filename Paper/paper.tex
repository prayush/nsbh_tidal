...
\red{Add segway into multiple observations. Point out that all NSs share the same
$\lambdans(\mns)$, so multiple signals can be combined the measure the common $\lambdans$.}

\textbf{Multiple identical sources at low SNR: }\label{s2:identical_multiple}
% 
\red{Simplify presentation: remove (or 16) and (or $\rho=70$) etc. ADd sentence:
similarly, $16,\rho=10$ events are comparable to one $\rho=70$ event.}
% 
...
given by}
\red{..into $I$ shells of same radial thickness. The radius at the middle of the}
