...
\includegraphics[trim={11cm 0 0 0},width=2.7\columnwidth]{plots/TNMchirpBiasesOverCIWidths_CI90_0_Lambda_SNR}.pdf
...
\includegraphics[trim={11cm 0 0 0},width=2.7\columnwidth]{plots/TNEtaBiasesOverCIWidths_CI90_0_Lambda_SNR.pdf}\\
...
% 
% 
Here we show a crude calculation which tells us that approximately $8$ observations
...
with $\rho=50$. We follow the outline in Ref.~\cite{Markakis:2010mp}.
...
$\mathcal{R}\equiv\langle N\rangle$. Let us assume that we have $\mathcal{R}_d$ detections
per year per unit effective volume, where $\mathcal{R}_d$ can vary from $0.6-1000$ per
$\mathrm{Gpc}^3$ per year~\cite{Abadie:2010cfa}. Then 
\begin{equation}
\langle N\rangle = \int_0^{\deff^\mathrm{max}} 4\pi D^2 \mathcal{R}_d \D D = \frac{4\pi}{3} \mathcal{R}_d (\deff^\mathrm{max})^3,
\end{equation}
and
\begin{eqnarray}
 \langle \frac{1}{\sigma^2}\rangle &=& \left(\frac{D_0}{\sigma_0}\right)^2 \int_0^{\deff^\mathrm{max}} \frac{4\pi\mathcal{R}_d D^2 }{D^2}\D D\\
 &=& \left(\frac{D_0}{\sigma_0}\right)^2 4\pi \mathcal{R}_d \deff^\mathrm{max},
\end{eqnarray}
...
\begin{equation}\label{eq:rmsSigmaIdenticalSources}
 \sigma_{avg} \equiv \frac{1}{\sqrt{\langle1/\sigma^{2}\rangle}} = \frac{\sigma_0}{D_0} \deff^\mathrm{max} \frac{1}{3\mathcal{R}_d}.
\end{equation}
...
There are two main caveats to this calculation, (i) it applies to identical sources, 
which is astrophysically next to impossible; (ii) the average measurement error 
$\sigma_{avg}$ has been defined as in the first equality in 
