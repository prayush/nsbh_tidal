...
noise at different SNRs, and run MCMC on the injections using BBH 
templates (LEA+ with tidal terms set to zero). We perform this analysis for mass
ratios $q\in\{2,3,4,5\}$, i.e.
$\mbh\in\{2.7M_\odot, 4.05M_\odot, 5.4M_\odot, 6.75M_\odot\}$, and BH spins
$\chibh\in\{-0.5, 0, +0.5, +0.75\}$. We always use $\mns=1.35M_\odot$ and
$\chins=0$, but explore $\lambdans\in\{500, 800, 1000, 1500, 2000\}$.
...
binary parameters given BBH waveforms, we show the measured $90\%$ confidence
intervals for binary masses and spins in Fig.~\ref{fig:CIWidths90_Lambda_SNR}.
...
(ii) so is $\chibh$. (iii) Mass ratio $\eta$ is determined somewhat worse,
...
measurements unless we know how large the systematic biases are due to an
incomplete template model. To quantify this, we find it useful to define $\arr$
as the ratio of the systematic biases induced by unmodeled tidal effects in
templates, to the statistical uncertainties associated with the measurement
itself. If $\arr\gtrsim 1$, including the effect of NS's
tidal distortion (during inspiral) and possible disruption (close to merger)
would be imperative to yield reliable estimates of binary masses and spins
from its measured GW signal.
