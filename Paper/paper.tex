...
associated $90\%$ confidence intervals (as the height of filled region), shown
as a function of number of observations $N$. Solid horizontal line indicates
the true value of $\lambdans$. Dashed and dotted horizontal lines (a pair for each
line-style) demarcate $\pm 25\%$ and $\pm 50\%$ error bounds.
}
...
\caption{%
This filled-region figure shows how our measurement of $\lambdans$ improves with
an increasing number of observed events $N$. Each color corresponds to an
independent populations with the true value of $\lambdans$ fixed as given in
the legend. For each population, we show the measured median (as filled
circles) and $90\%$ confidence intervals for $\lambdans$ measurement (as the
height of the filled region about the median). 
}
...
\caption{%
{\it Left:} This figure shows the fractional difference between the median
...
on $N$ for all populations, and (ii) information accumulates faster than
...
The fact that $\beta<1$ implies that the relative error
$\delta\lambdans/\lambdans$ decreases with increasing $\lambdans$, while the 
