...
A {\it coincident} detection of both GWs and gamma-rays from an NSBH merger will
...
% 
During early inspiral, the (weak) tidal field of the companion BH
deforms the star, thereby exciting its resonant oscillation modes. The induced
quadrupolar moment of the star is proportional to the tidal field of its companion,
% through the relation $Q_{ij} = -\lambda\mathcal{E}_{ij}$,
with $\lambdans$ the EoS-dependent proportionality constant that is also the
tidal deformability parameter, related to the NS's dimensionless Love number
