...
These waves draw energy from the orbit and increase the inspiral rate (as
...
neutron stars. The TOV equations therefore furnish a unique dependence of
$\lambdans$ on $\mns$. This fact enables us to go further - combine
information from multiple observations of realistic NSBH systems and
perform a fully-Bayesian analysis of how our estimation of $\lambdans$ changes
as we accumulate detections. This is similar to an earlier
...
sample different nuclear EoSs by sampling entire populations with different
values set for the tidal deformability of the NS.
% These values correspond to a hard EoS at the lower end, and a soft EoS at the upper.
Finally, we take source
locations to be uniformly distributed in spatial volume, and source
orientations to be uniform on a $2-$sphere. From an ensemble of such
populations, we learn the following.
% 
Our best estimate for $\lambdans$ improves much faster than our statistical
uncertainties. Within $10-20$ detections, the median measured value of
$\lambdans$ lies within $10\%$ of its actual value.
Measurement uncertainties for $\lambdans$, on the other hand, depend on
...
% 
Further, if the astrophysical ``mass-gap'' is real, we find that $20-50\%$
additional observations would be required to attain the same measurement
accuracy as above. Putter tighter constraints on the $\lambdans$ of a
population would require $50+$ NSBH observations, in either scenario.
% 
...
% \prayush{
In rest of the paper, we restrict our parameter space to span 
