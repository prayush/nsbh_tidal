...
section we present a fully Bayesian analysis of the effects of this
...
At design sensitivity, if we expect $10-300$ NSBH detections a 
year~\cite{Abadie:2010cfa}, {\it and} if BH mass values are uniformly likely
up to $\sim 35M_\odot$~\cite{LIGOVirgo2016a}, we can expect to detect $0.3-7$
{\it disruptive} NSBH mergers a year with $\rho\geq 20$, $0.07-2$ a year with
$\rho\geq 30$, and $0.04-1$ a year with $\rho\geq 70$. Therefore, the parameter
range spanned by our injections represents a physically interesting class of
systems that is {\it also} likely observable in the near future. 
...
As an illustration, in Fig.~\ref{fig:SingleSystemEtaPDFvsSNR} we show 
recovered probability distributions for binary mass ratio $\eta$, for NSBH
injections with $\mns=1.35M_\odot$, $\chins=0$, $\mbh=5.4M_\odot$, $\chibh=+0.5$
and $\lambdans=2000$, with the SNR ranging from $\rho=20$ (left) to
$\rho=50$ (right). In each panel, both the {\it true} and the {\it 
recovered} (median) values of $\eta$ are marked. The effect of ignoring tidal
corrections in templates manifests as a systematic shift of the measured $\eta$
value away from its true value. The associated {\it systematic} bias/error is
measured by the difference between the measured and true $\eta$ values. Darker
...
\textcolor{blue}{%
Credible intervals $(\Delta X)^{90\%}$ measuring the precision with which
$X=\{\mchirp,\eta,\chibh\}$ can be measured, are discussed in detail in
Appendix~\ref{as1:illustrations}.
}
...
will not hamper the measurement of non-tidal parameters from NSBH signals.
% 
We start with calculating $\arr_{\mchirp}$, and show it in
Fig.~\ref{fig:TN_chirpMassBias_vs_Lambda_SNR}. $\mchirp$ is the leading order
...
templates.
% 
For louder and less likely SNRs ($\rho\simeq 50$), we show that
...
% 
In addition to everything, we also note that $\arr_{\mchirp}\geq 0$ always,
...
measurement errors $\arr_\eta$ for the symmetric mass-ratio. We straight-away
notice that, unlike for $\mchirp$, the measured (median) value of $\eta$ is
always {\it lower} than its true value due to the lack of $\lambdans$-dependent
terms in filter templates. This also is so because templates with $\arr_\eta<0$
will have a lower merger frequency than the one with $\arr_\eta=0$, and will
therefore better fit a disruptive NSBH merger.
% 
...
$\eta$ measurement. The worst case is for the most deformable NSs
...
% 
This pattern is continued to even higher SNRs, as we can see from
Fig.~\ref{fig:TN_EtaBias_vs_Lambda_SNR}. We therefore conclude that, even if
under moderate restrictions on BH and NS parameters, $\rho=50$ is loud enough
...
associated with $\chibh$ measurement is shown in
...
% 
For the lowest considered SNR ($\rho=30$), $\chibh$ bias is about $2\times$
smaller than the statistical measurement uncertainty and is therefore mostly
negligible. Both can become comparable only when we have the most deformable NSs
in orbit around low-mass BHs. 
% 
Moving to higher SNRs, at $\rho\simeq50$ we find that systematics in $\chibh$
measurement begin to dominate for $\mbh\leq 4.5M_\odot$.
This suggests that we would need tidal templates to accurately
characterize astrophysical mass-gap violating NSBH binaries.
% 
At even higher SNRs, the systematics can completely dominate BH spin
measurement.
% 
From Fig.~\ref{fig:TN_BHspinBias_vs_Lambda_SNR} we also note that when signal
spins are the largest, $\chibh\times\arr_{\chibh}<0$. This is to be
expected because the median of the recovered posterior distribution for $\chibh$
can only get pushed inwards from the boundaries.
...
astrophysical NSBH binaries will remain limited by the intrinsic uncertainty due to 
...
binary mass and spin measurements become comparable to and can exceed the
uncertainty due to noise. Of the different parameters considered, we find that
amongst all non-tidal parameters, the measurement of $\eta$ degrades worst (in a 
relative-error sense) due to the use of BBH templates in deciphering an NSBH
signal.
...
is sampled uniformly from $[0,4000]$, and mass-ratio is additionally restricted to
$2\leq q\leq 6$.
...
\begin{figure*}
\centering
\textbf{Paradigm A \hspace{6cm} Paradigm B}\par\medskip
